		...
		for record in records:
			if cycle_week != (record['cycle'], record['week']):
				...
				if audio_widgets and (not spec or not spec.for_print):
					...
					with buttonstrip:
						t.button('♬', title = 'Musical score', onclick = 'window.open("%s","_blank");' % _aurl(filename_base + '.pdf?v=105'))
					...
					with buttonstrip_detail:
						t.audio(t.source(src = _aurl(filename_base + '.mp3?v=105'), type = 'audio/mpeg'), controls = True, id = filename_base)
					...
					with buttonstrip_accompanied_detail:
						t.audio(t.source(src = _aurl(filename_accompanied_base + '.mp3?v=105'), type = 'audio/mpeg'), controls = True, id = filename_accompanied_base)
...
def general(container, spec, records, show_cw):
	def render(record, container): # callback function, see _grammar_resources()
		with container:
			path = _geurl('%s/%s%s?v=105' % (record['download_path'], record['filename_crux'], record['filename_suffix']))
...
def geography(container, spec, records, show_cw):
	...
	def render(record, container): # callback function, see _grammar_resources()
		...
		if new_cw != cw:
			cw = new_cw
			path = 'c%dw%02d_geography.png?v=105' % (record['cycle'], record['week'])
...
def _add_eqality_record(table, record, left_field_name, right_field_name, youglishit = False, audio_base = None, line2 = None):
	...
	if audio_base:
		tr += t.td(
			...
			t.audio(t.source(src = _aurl(audio_base + '.mp3?v=105'), type = 'audio/mpeg'), controls = False, id = audio_base),
			right_text,
...
def _doc(title, css = None, scripts = None):
	...
	with d.head:
		...
		t.link(href = settings.k_static_url + 'css/main.css?v=105', rel = 'stylesheet')
	return d
...
def _event_formatted(record, for_print, timeline_sentences, detail_link = True):
	...
	if for_print:
		...
		final += t.audio(t.source(src = _aurl(filename_base + '.mp3?v=105'), type = 'audio/mpeg'), controls = False, id = filename_base)
		if detail_link:
